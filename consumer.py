...
def dashboard():
    ...
    # === ALERTY PRZEKROCZENIA NORM ===
    alert_msg = ""
    thresholds = {
        "Benzen": 5,
        "Dwutlenek azotu": 200,
        "Dwutlenek siarki": 350,
        "Ozon": 180,
        "Pył zawieszony PM10": 50,
        "Pył zawieszony PM2.5": 35,
        "Tlenek węgla": 10000
    }

    prog = thresholds.get(parametr)
    if prog and not df_city.empty:
        high_values = df_city[df_city["value"] > prog]
        if not high_values.empty:
            alert_msg = f"<div style='color:red; font-weight:bold; font-size:1.2em;'>⚠️ ALERT: Wysokie stężenie {parametr} w {miasto}!<br>({{len(high_values)}} pomiarów przekroczyło próg {{prog}} µg/m³)</div>"
